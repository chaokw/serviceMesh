...
func NewSever(rg registry.Registry, handler http.Handler, opts ...ServerOption) *Server { //chaokw
	...
	go func() {
		...
		case sig := <-ch:
			...
			if err = s.Stop(); err != nil {
				fmt.Printf("Server stop error :%v", err)
			}
		case <-s.exit:
		}

		if err := s.deregister(); err != nil {
			fmt.Printf("deregister error %v", err)
		}
	}()

	fmt.Printf("Http server listen on %s", s.opts.Address)
...
func (s *Server) register() error {
	...
	fmt.Printf("Registry [%s] name: [%s]\nregister node: [%s]\n", s.registry.String(), s.service.Name, s.service.Nodes[0].Id)
	...
	go func() {
		...
		for {
			...
			case <-t.C:
				if err := s.registry.Register(s.service, ttlOpt); err != nil {
					fmt.Printf("Server register error: %v", err)
				}
			case <-s.exit:
				t.Stop()
				return
...
func (s *Server) deregister() error {
	fmt.Printf("Registry [%s] deregister node: %s", s.registry.String(), s.service.Nodes[0].Id)
